...
public class PetController {
    ...
    public void createPet(PetType type) {
        ...
        try {
            ...
            view.showMessage(String.format("%d Добавлено! \n", res));
    ...
    public void updatePet(int id) {
        ...
        LocalDate birthday = LocalDate.parse(data[1], formatter);
        pet.setName(data[0]);
        pet.setBirthdayDate(birthday);
        try {
            int res = petRepository.update(pet);
            view.showMessage(String.format("Изменения сохранены! \n", res));
        } catch (RuntimeException e) {
            view.showMessage(e.getMessage()); }
    ...
    public boolean trainPet(int id, String command) {
        try {
            ...
            else if (!((PetRepository) petRepository).getCommandsById(id, 2).contains(command))
                view.showMessage("Команду нельзя выучить \n");
            else {
