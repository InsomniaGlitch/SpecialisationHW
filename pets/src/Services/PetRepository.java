...
public class PetRepository implements IRepository<Pet> {
    ...
    public List<Pet> getAll() {
        List<Pet> animal_nursery = new ArrayList<Pet>();
        Pet pet;
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            try (Connection dbConnection = getConnection()) {
                sqlSt = dbConnection.createStatement();
                SQLstr = "SELECT SpecieId, Id, PetName, Birthday FROM pet_list ORDER BY Id";
    ...
    public Pet getById(int petId) {
        ...
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            try (Connection dbConnection = getConnection()) {
                SQLstr = "SELECT SpecieId, Id, PetName, Birthday FROM pet_list WHERE Id = ?";
    ...
    public int create(Pet pet) {
        ...
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            try (Connection dbConnection = getConnection()) {
                SQLstr = "INSERT INTO pet_list (PetName, Birthday, SpecieId) SELECT ?, ?, (SELECT Id FROM pet_types WHERE Specie_name = ?)";
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLstr);
        ...
        try {
            ...
            try (Connection dbConnection = getConnection()) {
                if (commands_type == 1){
                    ...
                    SQLstr = "SELECT Command_name FROM commands c JOIN Specie_command gc ON c.Id = gc.CommandId WHERE gc.SpecieId = (SELECT SpecieId FROM pet_list WHERE Id = ?)"; }
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLstr);
                prepSt.setInt(1, petId);
                resultSet = prepSt.executeQuery();
                while (resultSet.next()) {
